...
namespace Tests
{
    public class BlockyRecursiveBacktrackerTest
    {
        static LilLogger logger = new LilLogger("BlockyRecursiveBacktrackerTest");

        [Test]
        public void GenerationShouldWork()
        {
            Vector2Int size = new Vector2Int(20, 20);
            IGridMap map = (new BlockyRecursiveBacktracker()).GenerateMap(size, 10);
